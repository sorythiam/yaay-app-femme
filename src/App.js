...
function MobileApp({ profile, tr, lang, setLang }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('app-changes-' + profile.id)
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications', filter: `woman_id=eq.${profile.id}` }, () => loadData())
      .subscribe()
  ...
  async function loadData() {
    ...
    const { data: notifs } = await supabase.from('notifications').select('*').eq('woman_id', profile.id).order('created_at', { ascending: false }).limit(50)
  ...
  return (
    <>
      <TopBar profile={profile} lang={lang} setLang={setLang} unreadCount={unreadCount} onBellClick={() => setTab('notifications')}/>
      {activeAlert && tab !== 'sos' && (
      ...
      <div style={{ flex: 1, overflowY: 'auto', background: '#FAF6F0' }}>
        {tab === 'home' && <HomeView profile={profile} pregnancy={pregnancy} appointments={appointments} exams={exams} notifications={notifications} tr={tr} lang={lang} setTab={setTab}/>}
      ...
      <BottomNav tab={tab} setTab={setTab} tr={tr} hasAlert={!!activeAlert} pendingCount={pendingRequests.length} lateExamsCount={lateExamsCount} unreadCount={unreadCount}/>
    </>
  )
}

function TopBar({ profile, lang, setLang, unreadCount = 0, onBellClick }) {
  return (
    <div style={{ padding: '8px 20px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(42,24,16,0.06)', background: '#FAF6F0', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button onClick={onBellClick} style={{ position: 'relative', padding: 6, background: unreadCount > 0 ? '#FFE8E2' : 'none', border: 'none', borderRadius: 10, cursor: 'pointer', fontSize: 18 }}>
          🔔
          {unreadCount > 0 && (
            <div style={{ position: 'absolute', top: 0, right: 0, minWidth: 16, height: 16, padding: '0 4px', background: '#C44536', color: '#FAF6F0', borderRadius: 8, fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #FAF6F0' }}>{unreadCount > 9 ? '9+' : unreadCount}</div>
          )}
        </button>
...
// NOTIFICATIONS VIEW
// =====================================================
function NotificationsView({ notifications, onChange, tr }) {
  ...
  return (
    <div style={{ padding: '20px 18px 100px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: 11, color: '#8B6F5C', fontWeight: 600, textTransform: 'uppercase' }}>Notifications</div>
          <div style={{ fontSize: 24, fontFamily: 'Georgia, serif', fontWeight: 600, color: '#2a1810', marginTop: 2 }}>Mes alertes</div>
        </div>
        {notifications.some(n => !n.is_read) && (
          <button onClick={markAllRead} style={{ padding: '6px 12px', background: '#F5F1EB', color: '#5D4037', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>✓ Tout marquer lu</button>
        )}
      </div>
      ...
      {notifications.length === 0 ? (
        <div style={{ marginTop: 40, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, color: '#8B6F5C' }}>Aucune notification pour le moment</div>
          <div style={{ fontSize: 12, color: '#B8A89A', marginTop: 6 }}>Vous serez alertée quand votre sage-femme modifie votre dossier.</div>
        </div>
      ) : (
        <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {notifications.map(n => (
            <button key={n.id} onClick={() => !n.is_read && markAsRead(n.id)} style={{ padding: 14, background: n.is_read ? '#FFFFFF' : '#FFF6E8', border: n.is_read ? '1px solid rgba(42,24,16,0.04)' : '2px solid #D4A574', borderRadius: 14, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', gap: 12, alignItems: 'flex-start', textAlign: 'left', width: '100%' }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: n.is_read ? '#F5F1EB' : '#F4E4C1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>
                {iconMap[n.type] || '💬'}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                  <div style={{ fontSize: 13, fontWeight: n.is_read ? 500 : 700, color: '#2a1810', lineHeight: 1.3 }}>{n.title}</div>
                  {!n.is_read && <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#C44536', flexShrink: 0, marginTop: 4 }}/>}
                </div>
                <div style={{ fontSize: 12, color: '#5D4037', marginTop: 4, lineHeight: 1.4 }}>{n.message}</div>
                <div style={{ fontSize: 10, color: '#B8A89A', marginTop: 6 }}>
...
// =====================================================
// BOTTOM NAV - 6 onglets avec notifications
// =====================================================
function BottomNav({ tab, setTab, tr, hasAlert, pendingCount, lateExamsCount, unreadCount = 0 }) {
  const items = [
